...
export default function LoginForm(){
  ...
  async function handleSubmit(e){
    ...
    try{
      setLoading(true);
      const res=await fetch("/api/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({nickname,username:nickname,password})});
  ...
  return(
    <div className="login-shell">
      <div className="login-credentials" aria-label="Демо данни за вход">
        <span className="login-credentials-label">Име:</span>
        <span className="login-credentials-value">Kai</span>
        <span className="login-credentials-separator" aria-hidden="true">•</span>
        <span className="login-credentials-label">Парола:</span>
        <span className="login-credentials-value">12345</span>
      </div>
      <section className="login-card">
        ...
        <form className="login-form" onSubmit={handleSubmit}>
          ...
          <button type="submit" className="login-button" disabled={loading}>{loading?"Влизане...":"Влез в системата"}</button>
        </form>

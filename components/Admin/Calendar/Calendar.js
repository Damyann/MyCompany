...
billCodeAdd,billCodeDelete,billCodeUpdate,saveCalendarTotalCfg,getCalendarDP,saveCalendarDP
}from"./Calendar_Api";
...
const TOTAL_KEYS=["Shifts","SICK","PH","Bonus","DP"],DEF_TOTAL=Object.fromEntries(TOTAL_KEYS.map(k=>[k,true]));
...
const setTotalCfgPersist=up=>setTotalCfg(prev=>{const next=typeof up==="function"?up(prev):up;const cfg=normTotalCfg(next);saveCalendarTotalCfg(ROLE,cfg).then(()=>{bustAndRefresh();}).catch(()=>{});return cfg;});
...
useEffect(()=>{const t=gridScrollRef.current;if(!t)return;const upd=()=>{const h=hScrollRef.current,hd=headScrollRef.current,n=namesRef.current;const namesW=n?.offsetWidth||0;const headW=hd?.scrollWidth||0;const base=Math.max(t.scrollWidth||0,namesW+headW);const diff=(h?.clientWidth||0)-(t.clientWidth||0);setHScrollW(Math.max(1,Math.ceil(base+diff)))};const id=requestAnimationFrame(upd);window.addEventListener("resize",upd);return()=>{cancelAnimationFrame(id);window.removeEventListener("resize",upd)}},[schedule,dc,sortedRows.length,daysInMonth]);
